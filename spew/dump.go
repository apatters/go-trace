...
func (d *dumpState) dump(v reflect.Value) {
	...
	default:
		var msg string
		if v.CanInterface() {
			msg = fmt.Sprintf("%v", v.Interface())
		} else {
			msg = fmt.Sprintf("%v", v.String())
		}
		fmt.Fprintf(d.w, msg)
	}
}
...
func fdump(cs *ConfigState, w io.Writer, a ...interface{}) {
	for _, arg := range a {
		if arg == nil {
			write(w, interfaceBytes)
			write(w, spaceBytes)
			write(w, nilAngleBytes)
			write(w, newlineBytes)
			continue
		}
...
func (d *dumpState) Write(p []byte) (n int, err error) {
	return write(d.w, p)
}
